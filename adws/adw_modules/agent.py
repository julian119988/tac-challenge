...
# Model selection map for slash commands
# Maps slash command to model for "base" and "heavy" model sets
...
def get_model_for_slash_command(
    slash_command: str, model_set: str = "base"
) -> str:
    """Get model for slash command based on model set.

    Args:
        slash_command: Slash command (e.g., "/implement")
    ...
    # Validate model_set
    if model_set not in ["base", "heavy"]:
        model_set = "base"

    # Look up in model map
    if model_set in SLASH_COMMAND_MODEL_MAP:
        return SLASH_COMMAND_MODEL_MAP[model_set].get(slash_command, default_model)
...
def execute_template(request: AgentTemplateRequest) -> AgentPromptResponse:
    ...
    If model is not explicitly set in the request, it will be auto-selected based on:
    1. model_set from ADWState (if working_dir provided and state exists)
    2. SLASH_COMMAND_MODEL_MAP for the specific slash command
    3. Default to "base" model set if no state found
    ...
    Example:
        request = AgentTemplateRequest(
            ...
            args=["plan.md"],
            adw_id="abc12345",
            model="sonnet"  # Explicitly set model (optional)
        )
    ...
    # Auto-select model if using default "sonnet" and state exists
    if model == "sonnet" and request.working_dir:
        # Try to load state to get model_set
        try:
            state_file = os.path.join(
                project_root, "agents", request.adw_id, "adw_state.json"
            )
            ...
            # If we can't load state, use default model
            pass
